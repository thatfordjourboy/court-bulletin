...
const GazettesHeader = () => {
  ...
  const linkStyles = "text-[#01292D] hover:text-[#71CED1] transition-all duration-300 hover:scale-105 text-base";
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-2">
            <nav className="flex items-center gap-6">
              <Link href="/gazettes" className={`${linkStyles} ${isActivePath('/gazettes') ? activeLinkStyles : ''}`}>
                Gazettes
              </Link>
              <Link href="/bulletin" className={`${linkStyles} ${isActivePath('/bulletin') ? activeLinkStyles : ''}`}>
                Bulletin
              </Link>
              <Link href="/cause-lists" className={`${linkStyles} ${isActivePath('/cause-lists') ? activeLinkStyles : ''}`}>
                Cause Lists
              </Link>
              {/* Notices Dropdown */}
              <div
              ...
              >
                ...
                {isNoticesMenuOpen && (
                  <div className="absolute left-0 mt-2 w-64 bg-white border border-gray-200 shadow-lg z-50">
                        ...
                        <div className="w-6 h-6 flex-shrink-0">
                          {notice.type === 'substituted-service' ? (
                            ...
                            <svg
                              viewBox="0 0 24 24"
                              fill="none"
                              className="w-full h-full text-[#01292D]"
                            >
              ...
              <Link href="/archives" className={`${linkStyles} ${isActivePath('/archives') ? activeLinkStyles : ''}`}>
                Archives
              </Link>
              <Link href="/statistics" className={`${linkStyles} ${isActivePath('/statistics') ? activeLinkStyles : ''}`}>
                Court Statistics
              </Link>
            </nav>
            ...
            <div className="flex items-center gap-4">
              <Link
                href="/contact"
                className="px-6 py-2 text-[#01292D] border border-[#01292D] hover:bg-[#01292D] hover:text-white transition-all duration-300 rounded-none"
              >
                Contact us
              </Link>
              <Link
                href="/signup"
                className="px-6 py-2 bg-[#01292D] text-white hover:bg-[#71CED1] transition-all duration-300"
              >
                Sign up
              </Link>
            </div>
        ...
        {isMobileMenuOpen && (
          <nav className="lg:hidden mt-4 pb-4">
            <div className="flex flex-col space-y-4">
              <Link href="/gazettes" className={linkStyles}>Gazettes</Link>
              <Link href="/bulletin" className={linkStyles}>Bulletin</Link>
              <Link href="/cause-lists" className={linkStyles}>Cause Lists</Link>
              <Link href="/notices" className={linkStyles}>Notices</Link>
              {noticeTypes.map((notice) => (
                <button
                  key={notice.type}
                  onClick={() => handleNoticeTypeClick(notice.type)}
                  className="pl-4 text-left text-xs text-gray-700 hover:text-[#01292D] py-1"
                >
                  {notice.name}
                </button>
              ))}
              <Link href="/archives" className={linkStyles}>Archives</Link>
              <Link href="/statistics" className={linkStyles}>Court Statistics</Link>
              <Link
                href="/contact"
                className="px-6 py-2 text-sm text-[#01292D] border border-[#01292D] hover:bg-[#01292D] hover:text-white transition-all duration-300 text-center mt-4"
              >
                Contact us
              </Link>
              <Link
                href="/signup"
                className="px-6 py-2 text-sm bg-[#01292D] text-white hover:bg-[#71CED1] transition-all duration-300 text-center"
              >
                Sign up
              </Link>
            </div>
